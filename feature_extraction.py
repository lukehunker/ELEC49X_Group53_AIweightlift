...
# Args
# -----------------------------
parser = argparse.ArgumentParser(description="Extract pose keypoints from video using MMPose (robust tracking)")
...
# NEW: leg separation constraint
MIN_LEG_SEP_REL = 0.35   # min L/R separation as fraction of shoulder width (tune 0.25-0.50)
MIN_LEG_SEP_PX = 18.0    # fallback absolute min separation when shoulder width is bad
...
# -----------------------------
# (2) Side-profile guard for leg-separation constraint
# -----------------------------
# If the view is side-on, L/R hips (and shoulders) overlap in image-space and "leg separation"
# constraints will freeze updates. We disable that constraint when the subject looks side-on.
SIDEVIEW_HIPSEP_OVER_SW_THR = 0.55  # smaller => more likely side-view
SIDEVIEW_SW_PX_THR = 20.0           # tiny shoulder width => unreliable; treat as side-view-ish
...
def enforce_leg_separation(prev, cur):
    """
    If L/R joints collapse (track same leg), keep them separated by reverting the
    more suspicious joint back to prev.

    Uses shoulder width as a dynamic scale; falls back to MIN_LEG_SEP_PX.
    """
    if prev is None:
        return cur

    sw = shoulder_width(prev)
    thr = max(MIN_LEG_SEP_PX, MIN_LEG_SEP_REL * sw) if sw > 1e-3 else MIN_LEG_SEP_PX
    thr2 = thr * thr
...
def likely_side_view(k16):
    """
    Heuristic: if L/R hips are close compared to shoulder width, the view is likely side-on.
    In side-view, enforcing leg separation tends to freeze the pose.
    """
    ...
    hip_sep = np.nan
    if np.all(np.isfinite(k16[L_HIP])) and np.all(np.isfinite(k16[R_HIP])):
        d = k16[L_HIP] - k16[R_HIP]
        hip_sep = float(np.hypot(d[0], d[1]))

    if np.isfinite(hip_sep):
        return (hip_sep / sw) < SIDEVIEW_HIPSEP_OVER_SW_THR
...
while True:
    ...
    if chosen is None:
        ...
        # (2) enforce L/R leg separation ONLY when NOT side-view
        if not likely_side_view(chosen):
            chosen = enforce_leg_separation(prev_k16, chosen)
